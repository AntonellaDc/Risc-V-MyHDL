...
def clk_driver(clk, halfperiod):
    @always(halfperiod)
    def driver():
        clk.next = not clk
    return driver

def stimulus(clk,rst,aluIO):
    yield delay(5)
    rst.next = 0
...
def cosim(aluIO):
    ...
    os.system(cmd)
    return Cosimulation("vvp -m ./myhdl.vpi alu_riscv", input_a=aluIO.alu_a_i,input_b=aluIO.alu_b_i,selector=aluIO.alu_op_i,output_alu=aluIO.alu_p_o)
...
if __name__ == "__main__":
    ...
    aluIO = ALUPortIO()
    halfperiod = delay(5)
    clk_driver_inst = clk_driver(clk,halfperiod)
    stimulus_inst = stimulus(clk,rst,aluIO)
    cosim_inst = cosim(aluIO)
    
    sim = Simulation(clk_driver_inst,stimulus_inst, cosim_inst)
    sim.run(100)
